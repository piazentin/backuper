...
There is no HTTP server in this repository yet. When a second composition root is added (for example under `entrypoints/http/`), use the following conventions:

- **Parallel composition roots:** Add the new entrypoint beside the CLI under `entrypoints/`. Reuse [`create_backup_database`](src/backuper/entrypoints/wiring.py) (or extend shared wiring in the same module) and call **controllers** with injected ports—do not duplicate ad hoc construction such as `CsvBackupDatabase(CsvDb(...))`.
