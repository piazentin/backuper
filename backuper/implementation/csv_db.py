...
class CsvDb:
    ...
    def get_fs_objects_for_version(self, version: models.Version) -> List[models.FileSystemObject]:
        version_file = os.path.join(self._config.backup_dir, version.name + ".csv")
        with open(version_file, "r", encoding="utf-8") as file:
            return [_fileobject_db_to_model(row) for row in csv.reader(file, delimiter=",", quotechar='"')]

    def get_files_for_version(self, version: models.Version) -> List[models.FileEntry]:
        version_file = os.path.join(self._config.backup_dir, version.name + ".csv")
        with open(version_file, "r", encoding="utf-8") as file:
            return [_fileobject_db_to_model(row) for row in csv.reader(file, delimiter=",", quotechar='"') if row[0] == "f"]
