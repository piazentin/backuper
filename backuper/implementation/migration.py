...
def zip_hashed(bkp_file_name: str, hash: str):
    with ZipFile(bkp_file_name + ".zip", mode="x") as zipfile:
        zipfile.write(bkp_file_name, hash, compress_type=ZIP_DEFLATED)
...
def migrate_1_to_zip(backup_main_dir):
    ...
    for hash, filename in hashes2filename.items():
        ...
        filter_by_extension = (
            extension is None or extension.lower() not in bkp.ZIP_SKIP_EXTENSIONS
        )
        if (
            os.path.exists(bkp_filename)
            and os.path.getsize(bkp_filename) > bkp.ZIP_MIN_FILESIZE_IN_BYTES
            and filter_by_extension
        ):
